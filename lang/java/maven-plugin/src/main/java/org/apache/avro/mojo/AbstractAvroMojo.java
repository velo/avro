...
public abstract class AbstractAvroMojo extends AbstractMojo {
  ...
  /**
   * Determines whether or not to create support code for java serialization
   * introduced on AVRO-1502.
   *
   * @parameter default-value="true"
   */
  ...
  protected String pluginIdentification() {
    for (Plugin plugin : project.getBuildPlugins()) {
      if (plugin.getArtifactId().equals("avro-maven-plugin")) {
  ...
  public List<Conversion> conversions() {
    ...
    return Arrays.stream(extraLogicalTypes).map(classname -> loadClass(Conversion.class, "Conversion", classname))
  ...
  private <E> E loadClass(Class<E> targetType, String suffix, String classname) {
    ...
    try {
      ...
      throw new IllegalStateException("Conversion not found on classpath " + classname
          + " make sure class is public and added to plugin dependencies");
    }
    ...
    try {
      ...
      throw new IllegalStateException(
          "Unable to create new instance of " + classname + " make sure class have a public constructor");
    }
  }
  ...
  public Map<String, LogicalTypeFactory> logicalTypeFactories() {
    ...
    return Arrays.stream(extraLogicalTypes)
        .collect(Collectors.toMap(classname -> loadClass(LogicalType.class, "LogicalType", classname).getName(),
