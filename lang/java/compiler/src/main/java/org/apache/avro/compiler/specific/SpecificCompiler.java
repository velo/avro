...
public class SpecificCompiler {
  ...
  public SpecificCompiler(Protocol protocol, DateTimeLogicalTypeImplementation dateTimeLogicalTypeImplementation) {
    ...
    // enqueue all types
    for (final Schema s : protocol.getTypes()) {
      enqueue(s);
    }
  ...
  /**
   * Set to false to not create support code for java serialization introduced on
   * AVRO-1502.
   */
  ...
  public Collection<String> getUsedConversionClasses(Schema schema) {
    final LinkedHashMap<String, Conversion<?>> classnameToConversion = new LinkedHashMap<>();
    for (final Conversion<?> conversion : specificData.getConversions()) {
      classnameToConversion.put(conversion.getConvertedType().getCanonicalName(), conversion);
    }
    final Collection<String> result = new HashSet<>();
    for (final String className : getClassNamesOfPrimitiveFields(schema)) {
  ...
  static class OutputFile {
    ...
    File writeToDestination(File src, File destDir) throws IOException {
      final File f = new File(destDir, path);
  ...
  public static void compileSchema(File[] srcFiles, File dest) throws IOException {
    ...
    for (final File src : srcFiles) {
      ...
      final SpecificCompiler compiler = new SpecificCompiler(schema, DateTimeLogicalTypeImplementation.DEFAULT);
      compiler.compileToDestination(src, dest);
    }
  }
  ...
  Collection<OutputFile> compile() {
    final List<OutputFile> out = new ArrayList<>();
    for (final Schema schema : queue) {
  ...
  public void compileToDestination(File src, File dst) throws IOException {
    for (final Schema schema : queue) {
      final OutputFile o = compile(schema);
  ...
  OutputFile compileInterface(Protocol protocol) {
    ...
    for (final Object velocityTool : additionalVelocityTools) {
      final String toolName = velocityTool.getClass().getSimpleName().toLowerCase();
      context.put(toolName, velocityTool);
    }
    final String out = renderTemplate(templateDir + "protocol.vm", context);

    final OutputFile outputFile = new OutputFile();
    final String mangledName = mangle(protocol.getName());
  ...
  protected void validateRecordForCompilation(Schema record) {
    ...
    if (!this.createAllArgsConstructor) {
      final Logger logger = LoggerFactory.getLogger(SpecificCompiler.class);
  ...
  OutputFile compile(Schema schema) {
    ...
    final OutputFile outputFile = new OutputFile();
    final String name = mangle(schema.getName());
  ...
  private Protocol addStringType(Protocol p) {
    ...
    final Protocol newP = new Protocol(p.getName(), p.getDoc(), p.getNamespace());
    final Map<Schema, Schema> types = new LinkedHashMap<>();

    for (final Map.Entry<String, Object> a : p.getObjectProps().entrySet()) {
      newP.addProp(a.getKey(), a.getValue());
    }

    // annotate types
    final Collection<Schema> namedTypes = new LinkedHashSet<>();
    for (final Schema s : p.getTypes()) {
      namedTypes.add(addStringType(s, types));
    }
    newP.setTypes(namedTypes);

    // annotate messages
    final Map<String, Message> newM = newP.getMessages();
    for (final Message m : p.getMessages().values()) {
      newM.put(m.getName(),
  ...
  private Schema addStringType(Schema s, Map<Schema, Schema> seen) {
    ...
    case RECORD:
      ...
      for (final String alias : s.getAliases()) {
        result.addAlias(alias, null); // copy aliases
      }
      seen.put(s, result);
      final List<Field> newFields = new ArrayList<>();
      for (final Field f : s.getFields()) {
        final Schema fSchema = addStringType(f.schema(), seen);
        final Field newF = new Field(f, fSchema);
        newFields.add(newF);
      }
      result.setFields(newFields);
      break;
    case ARRAY:
      final Schema e = addStringType(s.getElementType(), seen);
      result = Schema.createArray(e);
      break;
    case MAP:
      final Schema v = addStringType(s.getValueType(), seen);
  ...
  public boolean isStringable(Schema schema) {
    final String t = getStringType(schema);
  ...
  private String javaType(Schema schema, boolean checkConvertedLogicalType) {
    if (checkConvertedLogicalType) {
      final String convertedLogicalType = getConvertedLogicalType(schema);
    ...
    case UNION:
      final List<Schema> types = schema.getTypes(); // elide unions with null
      if (types.size() == 1)
  ...
  private String getConvertedLogicalType(Schema schema) {
    final Conversion<?> conversion = specificData.getConversionFor(schema.getLogicalType());
    if (conversion != null) {
  ...
  public String javaSplit(String s) throws IOException {
    final StringBuilder b = new StringBuilder("\""); // initial quote
    for (int i = 0; i < s.length(); i += maxStringChars) {
      if (i != 0) {
        b.append("\",\""); // insert quote-comma-quote
      }
      final String chunk = s.substring(i, Math.min(s.length(), i + maxStringChars));
  ...
  public static String mangle(String word, Set<String> reservedWords, boolean isMethod) {
    if (word.contains(".")) {
      ...
      // classname
      final int lastDot = word.lastIndexOf(".");
      final String packageName = word.substring(0, lastDot + 1);
      final String className = word.substring(lastDot + 1);
  ...
  private static String generateMethodName(Schema schema, Field field, String prefix, String postfix) {
    ...
    final String conflictingFieldName = (Character.isLowerCase(firstChar) ? Character.toUpperCase(firstChar)
        : Character.toLowerCase(firstChar)) + (field.name().length() > 1 ? field.name().substring(1) : "");
    final boolean fieldNameConflict = schema.getField(conflictingFieldName) != null;

    final StringBuilder methodBuilder = new StringBuilder(prefix);
    final String fieldName = mangle(field.name(),
  ...
  public List<Schema> toSchemaList(Schema schema) {
    if (overloadSetters && schema.getType().equals(Schema.Type.UNION)) {
